...
import { FLAVORS, getFlavorBySlug, SUPPLEMENT_FACTS, BENEFITS, CHIPS, SINGLE_PRICE } from '@/data/flavors'
...
export default function ProductDetail() {
  ...
  if (!flavor) return <Navigate to="/products" replace />
  ...
  return (
    <>
      <Seo
        title={`${flavor.name} — ZYRO Energy`}
        description={flavor.description}
        path={`/products/${flavor.slug}`}
      />

      <section className="pt-32">
        <Container className="grid gap-12 pb-20 lg:grid-cols-2 lg:gap-20">
          <div className="overflow-hidden rounded-2xl border border-zyro-border">
            <img
              src={flavor.hero}
